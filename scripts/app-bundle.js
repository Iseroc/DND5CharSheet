...
define('text!app.html', ['module'], function(module) { module.exports = "<template><require from=\"./normalize.css\"></require><require from=\"./styles.css\"></require><require from=\"./resources/fonts/font-awesome.min.css\"></require><nav class=\"navbar\" role=\"navigation\"><div class=\"navbar-placeholder\"></div><a class=\"navbar-item ${row.isActive ? 'active' : ''}\" repeat.for=\"row of router.navigation\" href.bind=\"row.href\">${row.title}</a></nav><div class=\"generalinfo\"><h1>${data.character.name}</h1><div class=\"saveAndLoadButtons\"><button click.delegate=\"data.save()\">Save</button> <input type=\"file\" ref=\"fileinput\" change.delegate=\"data.loadFile(fileinput)\" style=\"display:none\"> <button click.delegate=\"data.load(fileinput)\">Load</button></div></div><div class=\"content\"><router-view></router-view></div></template>"; });
...
define('text!components/inventory/inventory.html', ['module'], function(module) { module.exports = "<template><require from=\"./inventory.css\"></require><require from=\"./newItem/newItem\"></require><div class=\"itemArea\"><table class=\"itemTable\"><tr><th>Equipped</th><th>Attuned</th><th>Quantity</th></tr><tr repeat.for=\"item of data.inventory.equipped\"><td><span class=\"itemName\">${item.name}</span></td><td>-</td><td>-</td><td><div if.bind=\"item.equippable\"><button click.delegate=\"unequip(item)\">Unequip</button> <button click.delegate=\"editItem(item)\">Edit</button></div></td></tr></table></div><div class=\"itemArea\"><h3>Backpack</h3><ul><li repeat.for=\"item of data.inventory.backpack\">${item.name}<div if.bind=\"item.equippable\"><button click.delegate=\"equip(item)\">Equip</button> <button click.delegate=\"editItem(item)\">Edit</button></div></li></ul></div><div class=\"functionsArea\"><button class=\"btn-newItem\" click.delegate=\"startCreatingNewItem()\">Create a new item</button></div><new-item state.two-way=\"creatingNewItem\"></new-item></template>"; });
